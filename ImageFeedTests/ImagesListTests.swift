...
final class ImagesListTests: XCTestCase {
    ...
    func testCreateModelFromPhoto() {
        ...
        // when
        let model = presenter.modelForCell(at: IndexPath(row: 1, section: 0))
...
final class ImagesListViewControllerSpy: ImagesListViewControllerProtocol {
    ...
    func showSingleImageVC(_ vc: ImageFeedList.SingleImageViewController) {
        showSingleImageVCCalled = true
    }

    func reloadRows(at indexPaths: [IndexPath]) { reloadRowsDidCalled = true }

    func showProgressHUD() { showProgressHUDCalled = true }

    func dismissProgressHUD() { dismissProgressHUDCalled = true }
}
...
final class ImagesListServiceStub: ImagesListServiceProtocol {
    ...
    func changeLike(photoId: String, isLiked isLike: Bool, _ completion: @escaping (Result<Void, Error>) -> Void) {
        completion(.success(()))
    }
}
